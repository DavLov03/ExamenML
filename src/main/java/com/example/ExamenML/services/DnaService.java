...
import com.example.ExamenML.repositories.DnaRepository;
...
public class DnaService {
    ...
    private static int checkRows(String[] dna, int n) {
        ...
        for (int i = 0; i < n; i++) { // Cambiar 1 a n
            int count = 1;
            for (int j = 1; j < n; j++) {
    ...
    public boolean analyzeDna(String[] dna) {
        ...
        // Verificamos si el ADN ya existe en la BD
        Optional<Dna> existingDna = dnaRepository.findByDna(dnaSequence);
        if (existingDna.isPresent()) {
            // Si el ADN fue analizado, retornamos el resultado
            return existingDna.get().isMutant();
        }

        // Determinamos si el ADN es mutante y lo guardamos en la base
